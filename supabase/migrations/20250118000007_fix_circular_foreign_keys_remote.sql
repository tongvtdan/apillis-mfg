-- Migration: Fix Circular Foreign Key Constraints Between Users and Organizations (Remote)
-- This migration resolves the circular dependency issue that prevents proper data restoration
-- and future data insertion on the remote database.
...
-- Temporarily disable triggers to avoid conflicts during data updates
ALTER TABLE users DISABLE TRIGGER users_insert_trigger;
ALTER TABLE organizations DISABLE TRIGGER organizations_insert_trigger;
...
DO $$
BEGIN
    RAISE NOTICE 'Remote migration completed: Circular foreign key constraints between users and organizations have been resolved';
