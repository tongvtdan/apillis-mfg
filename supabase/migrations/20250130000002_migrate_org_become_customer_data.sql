-- Migration: Organizations as Customers - Phase 2: Data Migration
...
INSERT INTO activity_log (
    ...
    uuid_generate_v4(),
    jsonb_build_object(
        'migration', 'organizations_as_customers_phase_2',
        'step', 'create_customer_organizations',
        'organizations_created', (SELECT COUNT(*) FROM organizations WHERE description = 'Customer Organization'),
        'timestamp', NOW()
    )
);
...
INSERT INTO activity_log (
    ...
    jsonb_build_object(
        'migration', 'organizations_as_customers_phase_2',
        'step', 'update_contacts_organization',
        'contacts_updated', (SELECT COUNT(*) FROM contacts WHERE type = 'customer' AND description LIKE 'Primary contact for%'),
        'timestamp', NOW()
    )
);
...
INSERT INTO activity_log (
    ...
    jsonb_build_object(
        'migration', 'organizations_as_customers_phase_2',
        'step', 'update_projects_customer_organization',
        'projects_updated', (SELECT COUNT(*) FROM projects WHERE customer_organization_id IS NOT NULL),
        'timestamp', NOW()
    )
);
...
-- Log the contact points created
INSERT INTO activity_log (
    organization_id,
    action,
    entity_type,
    ...
    (SELECT id FROM organizations WHERE slug = 'factory-pulse-vietnam' LIMIT 1),
    'MIGRATION',
    'project_contact_points',
    uuid_generate_v4(),
    jsonb_build_object(
        'migration', 'organizations_as_customers_phase_2',
        'step', 'create_project_contact_points',
        'contact_points_created', (SELECT COUNT(*) FROM project_contact_points),
        'timestamp', NOW()
    )
);
...
-- Add additional contact points for testing (optional)
-- This creates secondary contacts for some organizations to demonstrate the new model
