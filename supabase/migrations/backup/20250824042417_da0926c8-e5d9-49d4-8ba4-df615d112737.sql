...
FROM (SELECT id, title, priority, assignee_id FROM public.projects WHERE status IN ('order_confirmed', 'procurement_planning', 'in_production') ORDER BY random() LIMIT 8) p;