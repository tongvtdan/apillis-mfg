...
-- Insert sample data for Production Orders
INSERT INTO public.production_orders (production_number, project_id, status, priority, planned_start_date, planned_end_date, quantity, assigned_to, notes)
SELECT 
  'PROD-2025-' || LPAD((ROW_NUMBER() OVER())::TEXT, 3, '0'),
  p.id,
  CASE 
    WHEN random() < 0.3 THEN 'planned'
  ...
  now() + (random() * 21 + 7 || ' days')::INTERVAL,
  (random() * 10 + 1)::INTEGER,
  p.assignee_id,
  'Production order for project: ' || p.title
FROM (SELECT id, title, priority, assignee_id FROM public.projects WHERE status IN ('inquiry_received', 'technical_review', 'supplier_rfq_sent', 'quoted') ORDER BY random() LIMIT 8) p;