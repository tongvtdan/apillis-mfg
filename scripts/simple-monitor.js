#!/usr/bin/env node
// Organization-Based Customer Model Migration - Simple Monitoring Dashboard
// Simplified monitoring to avoid complex query issues
...
class SimpleMigrationMonitor {
    ...
    async getBasicMetrics() {
        try {
            ...
            console.error('Error fetching basic metrics:', error.message);
            return null;
        }
    }
    ...
    async getMigrationProgress() {
        try {
            ...
            // Get migrated projects count
            const { count: migratedProjects, error: migratedError } = await supabase
                .from('projects')
                .select('*', { count: 'exact', head: true })
                .not('customer_organization_id', 'is', null);
            ...
            return {
                ...
                migrated: migratedProjects || 0,
                percentage: totalProjects > 0 ? ((migratedProjects / totalProjects) * 100).toFixed(1) : 0
            };
        } catch (error) {
    ...
    async displayDashboard() {
        ...
        // Database Metrics
        const dbMetrics = await this.getBasicMetrics();
        if (dbMetrics) {
        ...
        console.log(`   Projects Migrated: ${progress.migrated}/${progress.total} (${progress.percentage}%)`);
        console.log(`   Migration Status: ${progress.percentage > 80 ? '✅ Nearly Complete' : progress.percentage > 50 ? '⚠️  In Progress' : '🔄 Early Stage'}`);
        console.log('');
...
// Main execution
async function main() {
    const monitor = new SimpleMigrationMonitor();
...
Usage: node scripts/simple-monitor.js [options]
...
Examples:
  ...
  node scripts/simple-monitor.js --once
  `);
    process.exit(0);
}
