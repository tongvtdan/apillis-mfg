...
/**
 * Final script to fix UID mismatch by temporarily disabling foreign key checks
 * This script will complete the UID alignment for all remaining users
 */
...
async function fixUidMismatchFinalWithFkDisable() {
    try {
        console.log('🔧 Starting final UID mismatch fix with FK checks disabled...\n');
        ...
        const { error: disableError } = await supabase.rpc('disable_fk_checks_temporarily');
        if (disableError) {
            console.log(`   ⚠️  Warning: Could not disable FK checks: ${disableError.message}`);
        } else {
            console.log('   ✅ Foreign key checks disabled');
        }

        // Step 2: Update all foreign key references
        console.log('\n🔧 Step 2: Updating all foreign key references...');
        ...
        const { error: enableError } = await supabase.rpc('enable_fk_checks_temporarily');
        if (enableError) {
            console.log(`   ⚠️  Warning: Could not re-enable FK checks: ${enableError.message}`);
        } else {
...
// Run the script
fixUidMismatchFinalWithFkDisable();
